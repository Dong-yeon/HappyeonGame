...
- **디자인**: 도트(픽셀아트) 캐릭터 — 코드로 생성한 픽셀 스프라이트(**종족별 고유 도트** 7종 + 단계별 형태 + **궁극체 전용 도트 7종** + 인간 병사·장수). 렌더 시 **자동 외곽선 + 상단 림 하이라이트 + 접지 음영 + 바닥 그림자 + 숨쉬기 바운스**로 입체감 있는 상업적 품질. 밤하늘 야경(달·별·산), 타격/처치 이펙트, 먹색·금박 UI 테마
